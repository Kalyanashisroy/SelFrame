...
public class CustomReporter implements IReporter{
	...
	private String reportTitle= "Customized Report for JENKIN";
	//removing the .html from jenkin-report . A method is created to read the filename from properties file.
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
			String outdir1) {
		// added by bivas. REad the folder name from properties file
		String outdir= outdir1+"/"+getReportFolderName();
		//System.out.println("biasb ::"+outdir);
		//System.out.println("bivva ##"+getDateAsStringForHtmlNaming());
		try {
		...
		/** call pie chart created by bivas */
		findNumberOfTestsPassFailSkipped_and_makePieChart(suites); 
		...
		writer.flush();
		writer.close();
		// copy the Gstatic folder and loader script 
		copyGstaticfolderAndLoaderScriptPaste();
	}

	protected PrintWriter createWriter(String outdir) throws IOException {
		new File(outdir).mkdirs();
		//return new PrintWriter(new BufferedWriter(new FileWriter(new File(outdir, reportFileName))));
		//appending datetime to html file by bivas. 
		return new PrintWriter(new BufferedWriter(new FileWriter(new File(outdir, ( getReportHtmlFileName()+getDateAsStringForHtmlNaming()+".html") ))));
	}
	...
	/** To create pie chart added by bivas.   */
	protected void makePieChart(PrintWriter out,int passVar,int failVar,int skipVar) {
		out.println("<div id=\"piechart\"></div>");
		out.println("<script type=\"text/javascript\" src=\"https://www.gstatic.com/charts/loader.js\"></script>");
		out.println("<script type=\"text/javascript\" src=\"loader.js\"></script>");
		out.println("<script type=\"text/javascript\">");
		out.println("<!--Load google charts-->");
		out.println("google.charts.load('current', {'packages':['corechart']});");
		out.println("google.charts.setOnLoadCallback(drawChart);");
		out.println("<!--Draw the chart and set the chart values-->");
		out.println("function drawChart() {");
		out.println("var data = google.visualization.arrayToDataTable([");
		out.println("['Task', 'Total Testcases'],");
		out.println("['PASS', "+passVar+"],");
		out.println("['FAIL', "+failVar+"],");
		out.println("['SKIPPED',"+skipVar+"],");
		out.println("]);");
		out.println("<!--Optional; add a title and set the width and height of the chart-->");
		out.println("var options = {'title':'Test Status', 'width':550, 'height':550};");
		out.println("<!--Display the chart inside the <div> element with id=\"piechart\"-->");
	...
	/** To create pie chart added by bivas.   */
	public void findNumberOfTestsPassFailSkipped_and_makePieChart(List<ISuite> suites) {
		...
		// calling the piechart method -- by bivas		

		makePieChart(writer,qty_pass_m,qty_fail,qty_skip);
		//System.out.println("Passed : "+qty_pass_m);
		//System.out.println("Skipped : "+qty_skip);
		//System.out.println("Fail : "+qty_fail);
	}
	/*
	 * Method to get Date as String by bivas
	 */
	@SuppressWarnings("unused")
	private String getDateAsStringForHtmlNaming() {
		DateFormat dateFormat = new SimpleDateFormat("yyyyMMdd_HHmmss");
		Date date = new Date();
		return dateFormat.format(date);
	}

	private String getReportFolderName() {
		Helper help = new Helper();
		String folderName =help.getValueFrom("Report_Folder");
		return folderName;
	}
	private String getReportHtmlFileName() {

		Helper help = new Helper();
		String Html_fileName =help.getValueFrom("Html_fileName");

		return Html_fileName;
	}

	private void copyGstaticfolderAndLoaderScriptPaste() {
		String projectPath = System.getProperty("user.dir");
		System.out.println(projectPath);
		File source = new File(projectPath + "/src/com/FWL1/lib/gstatic");
		
		//File dest = new File(projectPath +"/target/surefire-reports/Jenkin_Report/gstatic");
		File dest = new File(projectPath +"/target/surefire-reports/"+getReportFolderName()+"/gstatic");
		
		//File dest2 = new File(projectPath+"/test-output/Jenkin_Report/gstatic");
		File dest2 = new File(projectPath+"/test-output/"+getReportFolderName()+"/gstatic");
		File sourcescript = new File(projectPath +"/src/com/FWL1/lib/loader.js");
		
		//File destscript = new File(projectPath +"/target/surefire-reports/Jenkin_Report/loader.js");
		File destscript = new File(projectPath +"/target/surefire-reports/"+getReportFolderName()+"/loader.js");

		//File destscript2 = new File(projectPath +"/test-output/Jenkin_Report/loader.js");
		File destscript2 = new File(projectPath +"/test-output/"+getReportFolderName()+"/loader.js");

		try {
			FileUtils.copyDirectory(source, dest);
			FileUtils.copyFile(sourcescript, destscript);
			FileUtils.copyDirectory(source, dest2);
			FileUtils.copyFile(sourcescript, destscript2);
